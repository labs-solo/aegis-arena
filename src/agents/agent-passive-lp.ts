...
///
/// Funding:
/// - Receives pre-split OKB + USD₮0 in correct ratio for full-range LP
/// - No initialization swap required — assets provided in exact LP proportions
/// - Computed from live sqrtPriceX96 of the pool
///
/// Income Streams:
...
///
/// Capital deployment:
/// - Full-range LP position: MIN_TICK (-887272) to MAX_TICK (887272)
/// - Balanced OKB + USD₮0 per current pool price
///
...
export class AgentPassiveLP extends BaseAgent {
  ...
  // Funding ready flag: tracks when both assets are available
  private fundingReady: boolean = false;
  ...
  // Dual-asset funding thresholds
  private readonly MIN_OKB_REQUIRED = ethers.parseEther('5'); // 5 OKB minimum for LP
  private readonly MIN_USDT_REQUIRED = BigInt(100) * BigInt(1e6); // 100 USD₮0 minimum for LP
  ...
  /// @notice Check if dual-asset funding is ready
  /// Returns funding status and current balances
  /// Requires: OKB ≥ 5 OKB + USD₮0 ≥ 100 USD₮0
  private async checkFundingReady(): Promise<{ 
    ready: boolean; 
    okbBalance: bigint; 
    usdtBalance: bigint;
  }> {
    ...
    const okbReady = okbBalance >= this.MIN_OKB_REQUIRED;
    const usdtReady = usdtBalance >= this.MIN_USDT_REQUIRED;
    
    return {
      ready: okbReady && usdtReady,
      okbBalance,
      usdtBalance,
    };
  }
  ...
  /// 1. Waiting for pre-split dual-asset funding (OKB + USD₮0 in correct proportions)
  ...
  async decideAction(state: GameState): Promise<Action[]> {
    ...
    // CHECK DUAL-ASSET FUNDING: Verify both OKB and USD₮0 are present
    const funding = await this.checkFundingReady();
    if (!funding.ready) {
      console.log('[PassiveLP] Waiting for dual-asset funding...');
      console.log(`  OKB: ${ethers.formatEther(funding.okbBalance)} (need ≥ 5)`);
      console.log(`  USD₮0: ${Number(funding.usdtBalance) / 1e6} (need ≥ 100)`);
      return []; // No actions until both assets are available
    }

    // Mark funding as ready to avoid repeated checks
    if (!this.fundingReady) {
      this.fundingReady = true;
      console.log('[PassiveLP] Dual-asset funding confirmed. Deploying LP position...');
    }
